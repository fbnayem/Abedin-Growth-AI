...
const newStats = `
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100 text-xs">
              ...
              <div className="text-center">
                <div className="text-[10px] text-purple-400 font-bold uppercase">Est. Reach</div>
                <div className="text-sm font-black text-purple-600 mt-0.5">
                  {Math.floor(camp.enrolledCount * 0.12)} <span className="text-[10px] font-semibold text-purple-400">(Proj.)</span>
                </div>
              </div>
            </div>
`;
