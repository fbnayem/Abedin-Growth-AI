...
> **Currency note — 2026-09-08.** This graph was derived at audit time and parts of it have been
> overtaken by remediation. The largest single change: **there is no longer a Firestore.** The
> document collections moved to the PostgreSQL instance, `server/firebase.ts` initialises
> authentication and nothing else, and no module imports the Firestore SDK
> (`scripts/check-no-firestore.mjs` enforces it). Rows below that describe a two-datastore split
> have been corrected in place and are marked; rows describing other defects are as they were at
> audit time and have NOT been re-derived, so treat an unmarked row as a claim about that date
> rather than about today. Section 1x of `addendum-status.md` records what moved.
...
| 2 | `server/firebase.ts` initialises | via import | ~~Uses the Firebase **client** SDK (`initializeClientApp` + `getFirestore`), gated only on `firebase-applet-config.json` existing on disk. No network I/O, so a "connected" Firestore handle proves nothing. Anonymous auth was removed because `firestore.rules` is fully open.~~ **CORRECTED 2026-09-08:** initialises `firebase-admin/auth` only, for ID-token verification, which needs no service account. No datastore handle of any kind. |
...
| **Outbox processing** | **None.** | **CORRECTED 2026-09-08 — the split is closed.** Producer, consumer and review console all reach the queue through `outboxService`, which writes the **PostgreSQL** `documents` table; one database, one transaction manager, and the claim runs under SERIALIZABLE with a single proven winner. What remains true from the original finding: there is still no lease, attempt counter or re-entrancy guard, and the relational `outbox_messages` table still has no writer. ~~Producer: `services/inboundPipeline.ts` inserts into **Postgres** `outboxMessages`. Consumer: `workers/outbox.worker.ts` reads **Firestore** `organizations/org_1/outbox`. Review console: `routes/outbox.routes.ts` lists / approves / rejects in **Postgres**.~~ | ~~**NO OWNER — SPLIT ACROSS TWO DATASTORES, AND INERT.**~~ The Firestore queue has no reachable producer, so `fetchPendingJobs` always returns `[]`. Approving a message writes to a store the worker never reads. `db` throws in dev, so all three review routes 500 and the worker marks every job `FAILED` before dispatch. There is no claim, lease, attempt counter, retry or dead-letter state anywhere. |
...
| `server/routes/outbox.routes.ts` | Human review console: list / approve / reject. **CORRECTED 2026-09-08:** goes through `outboxService`, so it reads and writes the same store the worker drains. |
...
| `outbox.service.ts` | **CORRECTED 2026-09-08:** the PostgreSQL document outbox — enqueue, atomic claim, mark processed/failed/requeued, with attribution. `orgId` is no longer hardcoded. |
...
| Module | One-line purpose |
|---|---|
| `server/workers/outbox.worker.ts` | 5-second `setInterval` polling the **PostgreSQL** outbox (corrected 2026-09-08). No re-entrancy guard, no lease, no atomic claim, no attempt counter, no retry. Every failure path terminates in `markFailed`. Its first per-job statement hits the throwing proxy. |
...
| `server/firebase.ts` | **CORRECTED 2026-09-08:** admin SDK for Auth only — ID-token verification, which needs no service account. No Firestore, no client SDK on the server. |
