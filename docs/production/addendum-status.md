...
~~**No section reaches `IMPLEMENTED_UNVERIFIED` or `VERIFIED`.** The repository contains two test files and **zero assertions**.~~

**Superseded 2026-09-08.** That was true when written. The repository now contains **62 test suites carrying 2,978 assertions**, a runner, a CI workflow, 20 guardrails, and a record of mutation testing against the real gate. The rubric above is unchanged and is what §1ac re-applies; only the sentence describing the tree is retracted. `server/tests/adversarial.test.ts:29-38` increments `passed++` unconditionally inside its `try` block and never inspects a return value, so `npm run test:adversarial` prints `Red Team Tests: 4/4 passed.` and exits 0 against any implementation, including a deleted one. `server/tests/pipeline.test.ts` swallows every error into `console.error`, is wired to no npm script, and imports a module proven dead. No test runner is installed (no vitest/jest/mocha in `package.json`) and no CI exists (no `.github` directory). The only gate that can fail is `lint` = `tsc --noEmit`, and `tsconfig.json` sets no `strict` flag; it exits 0 with zero diagnostics.
...
## 1ab. Six controls that existed and could not be reached (2026-09-08)

Every item here is the same shape, and it is the shape this audit keeps producing: **enforcement
that is real, careful, commented — and unreachable from the product.** A control nobody can
engage is worse than a missing one, because the enforcement makes it look present.

### The autonomy lock had a writer and no button (S38)

The previous section built the writer. Nothing called it: `grep -rn 'api/autonomy' src/`
returned zero. An operator watching the system draft the wrong thing to a customer could stop
the whole tenant or nothing.

Three gaps on one surface. The stop control had no button. **Approve claimed a send it would not
perform** — `outbox.worker` reads the lock immediately before dispatch and calls
`markFailed(..., terminal = true)`, and `markFailed` branches on `terminal` straight to
DEAD_LETTER, so approving on a paused conversation neither sends nor holds. And DEAD_LETTER rows
were filtered out of the console entirely, leaving S38's own `/requeue` reachable only by curl.

The console now shows the lock, offers pause and resume with a required reason, and says what
Approve will actually do. `approvalEffectFor(s) === 'SENDS'` is asserted to agree with
`mayProceed(s)` across every declared state, so the console and the enforcement cannot diverge.

### The opt-out this system could recognise and never offered (S26)

`List-Unsubscribe` appeared twice, both INBOUND: `automatedMail.ts` reads it off arriving mail
to decide somebody else's message is bulk. Nothing emitted one — so the gateway's
`contactData.unsubscribed === true` refusal was a check no recipient could ever satisfy.

An HMAC token per (tenant, contact); RFC 8058 one-click; GET serves a confirmation form and only
POST writes, because link scanners fetch every URL in a message before a human sees it. **No
working link means no send**: the gateway refuses `EMAIL_SEND` when it cannot mint one, because
the permissive reading — send anyway, omit the header — is the one that fails silently.

### The out-of-office no header would have revealed (S28)

An out-of-office setting no `Auto-Submitted`, no `X-Autoreply` and no `Precedence`, from a
personal address, is indistinguishable from a person. There is no header to add; the evidence is
not in the message, and matching subject prose is the substring classification this repository
already has a guardrail against.

RFC 3834 §2.1 answers this with a rate limit rather than a better classifier. A minimum interval
stops a fast loop; a count inside a rolling window catches a slow one. The reply history is
`number[] | null`, because `listByStatus` returns `[]` on failure — right for drawing a console,
an inversion here. Three signals that were genuinely missing were added: read receipts (`report-
type=disposition-notification`, which is not a bounce and so fell through every branch), `X-Loop`
and `X-Auto-Response-Suppress`.

### Attachments were recorded and never read (S17)

§1t made the walk record them; nothing consulted the result. Metadata alone shows plenty: an
executable extension anywhere in the name, a trailing dot or space that Windows removes on open,
a name disagreeing with its declared type, a direction override that renders an executable as a
PDF, and attachments the walk COUNTED but did not record. All route the draft to human review —
never dropping the message, which is evidence.

The other three quarters of this row are answered by a posture: **this system never downloads
attachment bytes.** Sniffing requires downloading first, which means holding prospect-supplied
binaries, which is what creates the need for a scanner. Guardrail 20 makes the absence a control.

### Three dead shadows of live controls (S1)

Found while completing S26, and none inert. `pipeline.service.ts` was a second inbound pipeline
with `mock_contact_id`. `suppression.service.ts` was its suppression check, reading an in-memory
array no real contact is ever in — so it answered "not suppressed" for every recipient, with a
`processUnsubscribe` ending in a `try {}` around a comment saying "This is a stub for the logic".
`killSwitch.controller.ts` was described as unreachable in `circuitBreaker.service.ts`'s own
header. All deleted, all kept deleted by a test.

Worse than dead: `tripCircuitBreaker` and `resetCircuitBreaker` had no call sites but were
IMPORTED by `server.ts` — in scope, one autocomplete from being called.
`resetCircuitBreaker()` set `globalAutonomousSendEnabled = true` without consulting the durable
state or `AUTONOMY_ENABLED`, defeating the kill switch's deliberately asymmetric design, and it
read like the obvious thing to call after fixing whatever tripped the breaker.

### The CSP had nowhere to report to (S35)

A policy with no reporting is a control whose only feedback channel is a customer saying the
page looks wrong. Both spellings are emitted — `report-uri` unconditionally because it is
relative and needs no configuration, `report-to` only when `APP_URL` gives an absolute origin,
since it is inert without one. The endpoint persists nothing, logs a bounded and neutralised
summary, and answers 204 to every body.

A bug the tests found: the Reporting API calls it `blockedURL`, and camelCasing `blocked-uri`
gives `blockedUri`. Every violation from a browser using the newer format would have logged as
"(no recognised fields)" — reporting that appeared to work and reported nothing.
`effective-directive` transforms correctly by coincidence, which is why the first test written
passed and the second did not.

### What this batch cost, and what it proved

81 mutants against `npm run verify`, in six rounds. Every survivor was either fixed or measured.
The survivors were more instructive than the kills:

- **Two were decisions inside an express handler and a `.tsx` component** — neither reachable
  under `environment: 'node'`. Both moved; neither needed a new assertion, only a place an
  assertion could reach. That is now the fourth and fifth time on this project.
- **One looked equivalent and was not.** Dropping a `hasOwnProperty` guard survived; the obvious
  reading (a function fails the `typeof` check) is true and incomplete. Measured both ways, one
  input separates them: a conversation whose id is `__proto__`, where assignment moves the
  prototype instead of adding a key.
- **One was output-equivalent by construction** — `timingSafeEqual` versus `!==`, identical over
  95 probed inputs — and is recorded as covered by a weaker mechanism rather than presented as
  covered.
- **One was my own mutant being too weak** to disable what it claimed to disable.

---

## 1ac. The re-grade — 49 rows, measured rather than remembered (2026-09-08)

**The instrument was two days stale, and by this document's own standard that made the project
incomplete regardless of what the code did.** §2.1 still read `VERIFIED 0 · PARTIAL 39 ·
NOT_STARTED 9` — the first pass's numbers, never recalculated across twenty-eight remediation
sections. §1 still closed with "The repository contains two test files and **zero assertions**",
which was true when written and is now off by 2,978. Every row in §3 carried its original
status, so S48 read `PARTIAL` while §1aa recorded the schema gate landing with its own suite.

A grading document that cannot say what is done is not a bookkeeping problem here. §1 is the
deliverable: *only executable proof of business invariants counts*. If the record of that proof
is wrong, there is no proof.

### What this pass did NOT do

It did not read the commit messages. §2 says "never mark VERIFIED because another agent claimed
it was complete", and the agent most likely to have overclaimed on these rows is me, in the
twenty-eight sections above. So the grade was rebuilt from three mechanical measurements, each
re-run against the working tree:

1. **Every greppable claim each row's gap text still makes**, re-run. Thirty-six of them. The
   question was not "was this fixed" but "is the sentence in the matrix still true".
2. **The structural claims** — how many endpoints are still inline, whether `src/` still
   value-imports a server agent, how many migrations have a rollback, whether a CI workflow
   exists, whether the retired tables have writers.
3. **Which test suite imports which production module, and how many assertions it carries.** A
   `VERIFIED` grade needs a suite that can be pointed at, not a belief that one exists.

### Four times the measurement corrected me, and one of them mattered

Six rows came back `STILL THERE` on the first pass — `|| 'sim_'`, `res.status(500).json({error:
e.message})`, `resolvedCountry = 'US'`, `isB2B: true`, the kill switch stub, `default: return
true`. Five were the comments that record the old defect, matched by a pattern that did not
strip them. **That is the same mistake this repository has now made four times** — on the
credential-file check, on the automated-mail classifier, on the CSP route's datastore check, and
here — and each time the shape is identical: a scan reads the fix's own explanation as the
defect, which is exactly what teaches people to delete the explanation.

The sixth was real and was not a defect. `isIrreversible` genuinely ends `default: return true`,
and there `true` is the safe direction: an ActionType nobody classified is treated as
irreversible. S46's finding was about the *dispatch gate's* default, which is fixed.

Three more probes returned zero and were wrong about it — `promptAssembly`, `emailKey` and the
`REAL_*` flags all looked dead because the patterns searched for the wrong identifier
(`assemblePrompt`, `normalizeEmailKey` and `isRealActionEnabled` are what the code actually
calls). **A probe returning nothing is a claim about the probe.** Recorded because two of those
would have downgraded a row to NOT_STARTED on the strength of a typo.

### The result

| State | Count | Change from the first pass |
|---|---:|---|
| `VERIFIED` | **36** | 0 → 36 |
| `IMPLEMENTED_UNVERIFIED` | **1** | 1 → 1 (S1 moved out, S10 moved in) |
| `PARTIAL` | **12** | 39 → 12 |
| `NOT_STARTED` | **0** | 9 → 0 |

### The twelve rows that are still PARTIAL, and exactly what is missing
...
| **S4** | The code half is closed: `organization_id NOT NULL` on every table, request-scoped tenant from a signed claim, `org_id` derived by the store from the path so it cannot disagree with it. What remains is **console-only** — `firestore.rules` is deny-all and deployable, and has not been deployed. Nothing in this repository reads that datastore any more (§1x), so the rules govern no code path here; the live instance is nonetheless still open. |
| **S5** | Seven migrations describe `schema.ts` and drift is detectable. **Zero rollback migrations exist**, and no test exercises a backfill. Expand/contract is a claim, not a demonstrated property. |
...
| **S39** | **72 endpoints are still declared inline in `server.ts`**; five routers are mounted. Down from ~75/0, and not the decomposition this row asks for. |
| **S40** | **Four React modules still value-import `server/agents/growthCommandAgent`.** The worst half is closed — no `src/` file imports `@google/genai` any more, so the API key cannot reach the bundle — but the direction is still inverted. |

### The one row that is IMPLEMENTED_UNVERIFIED

**S10 — audit logging fail-closed on the gateway.** `logAction` refuses rather than swallowing,
and the dispatch path depends on it. But the only suite importing the gateway for this property
is `providerResult.invariant.test.ts`, and it asserts the provider-result invariant rather than
...
For each of the 36: a named suite that **imports the module** and asserts the invariant, plus the
gap text's own claim re-run and found false. Where this hardening pass wrote the code, there is
also mutation evidence against `npm run verify` — 79 mutants across four rounds in the sessions
before this one, and **81 more in this one** (14 autonomy, 16 unsubscribe, 15 reply-loop, 16
attachment, 7 dead-schema, 13 CSP), every survivor either fixed or measured and recorded as
unexpressible.

Two are recorded as unexpressible rather than covered, and both are named at their call sites:
the constant-time comparison in `identityFromToken` (output-equivalent to `!==` over 95 probed
inputs; only wall-clock separates them) and a count floor in `deadSchema.invariant.test.ts`
(insurance, and no assertion detects insurance being removed before the insured event happens).

**`VERIFIED` is not a claim that a section is finished forever.** It is this document's own
definition: implemented on a live path, with an executable test asserting the business invariant
that is capable of failing. It says nothing about severity, and the severity counts in §2.1 are
deliberately left alone — a fixed CRITICAL was still a CRITICAL.
...
### S1 — the code graph, derived, and the three things it was written about

The graph this row is named for was written by hand at audit time and was stale within days: it
listed modules since deleted and omitted ten since written. A description of a tree is a second
copy of the tree, and second copies drift, so the graph is derived now. `server/build/codeGraph.ts`
follows every import from the two runtime entrypoints — relative, `@/` alias, dynamic; comments
blanked by the scanner the route table uses, because the trap recorded twice this week was a
comment stripper opening a block comment inside a string — and from every script that package.json
or a workflow under `.github/workflows` invokes. A module is live, operational-only or dead by what
reaches it. What the scanner cannot resolve it refuses: a specifier that resolves to nothing, or a
script entry naming a file that is not there, is an error, not an omission, because an omission
would report the tree as tidier than it is. The committed document is regenerated by the suite and
any difference fails the gate, the way the OpenAPI document's does.

The first regeneration named ten dead files, and the row's three parts were dealt with in turn.
**Dead modules:** eight agents nothing called — a campaign strategist, a meeting brief writer, the
second inbound pipeline's understanding, composer and technical agents, and three "discovery"
agents that asked a model to invent investors, leads and partners, which is not a feature to wire
under §18 — plus a jurisdiction policy nothing read and a component nothing rendered. Deleted, and
the dead set is held at zero from here: a file nothing reaches fails the gate, so the tree cannot
carry an undecided thing. Eight legacy prompt sites and eight `safeGenerateJSON` sites went with
them; both ratchets are lowered to what remains, two and one, and the abstention ratchet's own
rule — a baseline that no longer matches reality is a ratchet that has stopped — is why the gate
insisted. **Competing owners:** the suite pins the importers of each external capability (Gmail,
Calendar, the HTTP client, the model client, the gateway, the outbox) and of each outward SDK
(`@google/genai`, `stripe`, `firebase-admin`, `pg`) to an exact recorded set. The package table
found one on its first run: `pitchBattleAgent` constructed its own `GoogleGenAI` and called the
model directly — past `geminiClient`, so past S22's run log, S37's cost ledger and the
prompt-authority ratchet, which scans the client's callers and could not see a caller of the SDK
— and it interpolated the practising user's pitch into its system instruction. It calls through
the client now, with the transcript and the pitch as fenced blocks; a second SDK owner fails the
suite. **Unaccounted scripts:** every code file under `scripts/` must be named by a package.json
script or a workflow, or imported by one that is. Three operator tools that only their own header
comment knew how to run — the destructive database rebuild, the outbox version backfill, the
tenancy claim writer — are named now (`db:apply`, `outbox:backfill-version`, `org:claim`), and the
list of scripts reached by nothing is held at zero.

The graph also carries the invariant the repository-integrity finding asked for: every live and
operational module is tracked by git. It is the assertion `a391ad4` could not make at the time,
and the probe in the mutation run — an untracked file imported by `server.ts` — is the defect
itself, named by the test that fails.

What the graph does not claim: who ought to own a capability. It reports who does, and the suite
pins the answer somebody decided; a new importer of the Gmail adapter is a change to a list in a
test, made on purpose, rather than a side effect of a merge.

Live at commit e0481dc: a fresh `git clone` of HEAD into an empty directory — 532 tracked files, no `.env`, lockfile in sync with package.json (`npm ci --dry-run`) — compiled with zero type errors and passed the full gate there (84 suites, 2,110 tests) with nothing from the working tree, which is the assertion the integrity finding could not make at the time; a first attempt installed dependencies onto a system drive with 431 MB free and died of ENOSPC, so the second borrowed the working tree's install through a junction and tested the clone's own sources.

Mutation: 16/16 — 15 of 16 on the first run; the survivor, G3 (comments not blanked), was equivalent for every commented-out form the scanner test then used, since a line-start regex never matched `// import`; the test gained a block comment whose inner line begins with `import` and a commented-out dynamic `import()`, which the unblanked scan follows, and G3 was killed on the re-run; the sixteenth is the environmental probe, an untracked file imported by `server.ts`, which the git-tracked invariant failed on, naming the file.

### S39 — the monolith, decomposed, and what only a booted server could show

Seventy-two API routes were registered inline in `server.ts`, inside a single `startServer`
function two thousand lines long, and twenty-three suites had learned to find their evidence
there. The move is one mechanical pass — a script that cuts each registration with the comment
above it and the closer at its own indent, rewrites `app.post("/api/x/y"` as `xRouter.post('/y'`,
computes each router's imports from the identifiers it uses, and prunes `server.ts` down to what it
still uses — so the text of every handler is what it was, and a reviewer can diff the routers
against the old file. Fifteen routers were born; `server.ts` is 224 lines and does four things:
middleware, mounts, static serving, listen. The suites followed. Most now read a handler through
the route table's own reader, so a suite that pins "the meetings route calls `parseInstant`" also
pins that the meetings route lives in `meetings.routes.ts`, which is the thing S39 asks for.

The gate passed on the decomposed tree — 85 suites, every guardrail — and the booted server
refused every tenant-scoped request with `TENANT_REVOCATION_UNVERIFIABLE`. The pruning had
removed the named import of `server/config/safeMode` because the readiness route that used its
exports had moved, and that import was load-bearing for a reason the top of `server.ts` states in
capitals: it is the module that calls `dotenv.config()`, and `server/config/environment.ts` reads
`DATABASE_URL` as it evaluates. With the first import now the rate limiter, the store initialised
with no URL, came up null, and the middleware did what §14 asks of it — refused. Nothing in the
gate reads the environment at module-evaluation time in that order, so nothing in the gate could
see it; a request to a running process could. The import is a side-effect import now, and the
first import of `server.ts` is an assertion in the suite, with the reason beside it.

Two smaller things the move surfaced. The P1.13 pass converted sixteen `res.json({ success: true })`
stubs into refusals, and its guardrail looks for exactly that shape; eight handlers whose whole
body was a different fixed answer — `{ intentConfidence: 0.9 }`, `{ decision: "Proceed" }`,
`{ enabled: true }`, a placeholder sender identity — passed it, and one more echoed the request
back as though it had been saved. They refuse now, and the guardrail scans the routers and forbids
a handler whose every value is a literal, in the arrow form and the block form; the mutation run
plants one of each in a router file and the script fails naming the file. And the route table,
written for S11, followed a named handler only to `function fn(`; `setCampaignStatus` and
`setOpportunityStage` are `const fn = async (` arrows, so `POST /api/campaigns/:id/status`,
`/toggle`, and both verbs of `/api/pipeline/:id/stage` were read as their registration line and
reported as reading no body — absent from the no-schema list, which is the one place a body
reader without a contract is supposed to be unable to hide. They have contracts now: the state
named, an optional version, and nothing else, refused before the read. The behavioural test is the
status code: a bad body on a record that does not exist is a 400 and not a 404, which is only true
if the schema runs first.

Live at commit 0f286cf: the first boot after the move answered `TENANT_REVOCATION_UNVERIFIABLE` on every tenant-scoped route — the pruned `.env` loader, above; with the import restored, the booted server served `GET /api/health` from `server/routes/health.routes.ts:55` (200, the `x-source` in the served document), `GET /api/leads` and `GET /api/campaigns` 200, `POST /api/campaigns/nope/status` with `{status:"BOGUS"}` 400 and with `{status:"ACTIVE",extra:1}` 400 naming the key, `/toggle` with a legal state on a missing campaign 404, `PUT /api/pipeline/nope/stage` 400 then 404 likewise, the nine fixed answers 501 `NOT_IMPLEMENTED`, `POST /api/webhooks/gmail` without a token 401 `WEBHOOK_VERIFICATION_FAILED`, `POST /api/settings` with an unknown field still 400; the served coverage read 88 routes, 12 with a contract, 14 without, 2 raw.

Mutation: 12/13, then 2/2 — R5 (the fixed-answer patterns dropped from the guardrail) survived a text pin on the constant's name; the suite now runs the script against a planted file of its own, in both forms, and R5 was killed on the re-run beside a new mutant that moved the `.env` loader below the rate limiter import; the run also planted a fixed answer in a router file in each form and the script failed naming the file.

### S26 — the sequence runs, and only as far as the guards allow

Everything around a campaign existed and the campaign did not: fourteen guards on the gateway,
a recipient state machine with eleven states, a `campaign_recipients` table with the right unique
constraint, a campaign document carrying `steps[]` — and nothing that enrolled a contact, chose a
step, rendered it or advanced anyone. The row's own words were "the guards protect sequences
that cannot run". The engine is two modules. `campaignSequence.ts` is pure: which step is next,
when it is due, what the template becomes for this contact, where a recipient goes when the
outbox reports. `campaignEngine.service.ts` is the part that touches records: enrolment, the
tick, reconciliation, the stops.

Three decisions are worth their reasons. **Recipients are documents**, not rows in the table
declared for them: that table's foreign keys point at the relational `campaigns` and `contacts`
tables, and both campaigns and contacts are documents, so an enrolment could not have been
inserted without inventing a relational campaign to satisfy the key. The document's id is derived
from (campaign, contact), the device that made duplicate contacts unrepresentable, and the table
is retired in the schema with a note where somebody grepping for it will read it. **Advancement
is reconciliation**, not a hook: a SENDING recipient moves when a tick reads its job as
PROCESSED, and the next step is due its delay after the send the outbox CONFIRMED, not after the
tick that enqueued it; a tick that dies between enqueue and update is repaired by the next,
because the idempotency key is derived from (campaign, contact, step) and the outbox hands back
the job it already holds. **Every guard input is computed or absent**: the contact document's
flags, the recipient's own send history across campaigns for the cap and the cooldown, the other
enrolments for conflicts, the relational conversations for an open exchange or a pending draft,
the contact's stated time zone for the hour — and what cannot be computed is `undefined`, which
the guards treat as NOT RUN and refuse. The most common refusal in this tree is QUIET_HOURS on a
contact with no `timeZone`, which is the right answer for a person whose 3am we cannot tell; the
field exists now, IANA only, and a fixed offset is refused because it cannot follow daylight
saving. A contact with no consent record is refused before a job exists, because the gateway
would refuse the job after, and a queue of jobs that cannot go is noise an operator has to read.

What a tick does, in order: cancel the pending recipients of a campaign that is COMPLETED or
gone; reconcile SENDING recipients against their jobs; stop anyone suppressed or unsubscribed
since, and anyone who has written since enrolment (REPLIED once something was sent, CANCELLED
before, because the machine has no reply to something that was never sent); dispatch what is
due, oldest first, at most twenty-five per tick, held for HUMAN_REVIEW unless the campaign is
FULL_AUTOPILOT, stamped with the conversation's inbound version so the worker's integrity check
can refuse a draft that a reply has overtaken. The daily and per-domain limits count the sends
of the tick in progress, so a burst cannot exceed them inside one tick. The scheduler that runs
ticks on an interval is off unless `CAMPAIGN_SCHEDULER_ENABLED` is exactly "true"; the on-demand
tick reports the scheduler's state beside its own report, and `POST /api/autopilot/run-cycle-now`
— a fixed "success" until this week, then a refusal — now runs the thing it claimed to run.

Nothing here sends. A tick enqueues; the outbox worker dispatches through the gateway; the
gateway checks consent, suppression, capability, the sender's DNS posture and the Safe Rebuild
Mode flags, and `REAL_EMAIL_SEND_ENABLED` is false. What changed is that a sequence can run — a
recipient can be enrolled, come due, be refused for a reason that is written down, or be handed
to the outbox with everything the worker needs — which is a different fact from a message
leaving, and the row is graded on the first.

Live at commit 8b51919: the served document read 92 routes with 14 contracts, the enrolment contract carrying `minItems 1, maxItems 500, additionalProperties false`; `POST /api/campaigns/nope/recipients` refused `{}`, `{contactIds:[]}` and `{contactIds:["x"],sendNow:true}` with 400 naming the field, and `{contactIds:["x"]}` with 404 (no such campaign); `POST /api/contacts/nope/time-zone` refused `+06:00` with 400 (not an IANA zone) and `Europe/London` with 404 (no such contact); `POST /api/campaigns/run-tick` and `POST /api/autopilot/run-cycle-now` each ran a real tick over the live organisation — 0 campaigns, nothing reconciled, dispatched, refused or stopped, no errors — the second reporting the scheduler as not running with the reason (`CAMPAIGN_SCHEDULER_ENABLED` is not "true"), which the boot log also said; nothing was enrolled and no row was written.

Mutation: 17/17 — consent skipped, noon assumed for a contact with no zone, every step PENDING, the key without the step, the job without its inbound version, suppression not stopped, the last step reported as a wait, the next step scheduled from the tick rather than the confirmed send, a LinkedIn step treated as email, an unfilled tag replaced by nothing, the per-tick bound and the per-domain count of this tick's sends removed, the scheduler starting whatever the flag says, a second enrolment overwriting, a fixed-offset zone accepted, a reply not stopping the sequence, the guards evaluated and ignored.

### S4 — the database holds the tenant boundary, not only the code above it

The row's remainder named Firestore's open rule, and Firestore is no longer where anything
lives: the datastore has been PostgreSQL since §1u, nothing in the repository reads or writes
Firestore data, and the rules file in the tree is the closed rewrite whose deployment to the old
project is the owner's console step. What the remainder was really about survived the move
intact. The document store was tenant-safe by construction — the path names the organisation,
`org_id` is derived from it, every query carries it — and construction is a property of the code
that exists today. A query with a defective predicate, a maintenance script on the application's
credentials, a future Drizzle statement on `documents`: none of them would have been told by the
database that another tenant's rows are not theirs.

Migration 0009 tells them. A CHECK holds `org_id` equal to the tenant the path names, the
`tenantOf` rule written in SQL, so the column cannot be made to disagree with the path by any
writer. Row-level security on `documents`, with one policy: a connection sees, and may write,
only the rows of the tenant it has NAMED through `app.org_id`, plus the tenantless top-level
documents (`oauth_connections`, the organisations registry), and a connection that names no
tenant sees no tenant at all. It is FORCED, so the owning role is subject to it too; a superuser
bypasses every policy by definition, which is why the application role must never be one and
`db:verify` now reads that bit. The store names the tenant before every statement, from the
path it is about to touch — one transaction per operation, the setting local to it, and per
operation inside `runTransaction`, so a transaction that spans two tenants sees each only while
it names it. Nothing above the store changed: the same paths, the same queries, and a boundary
that no longer depends on them being right.

The proof runs on a real engine. PGlite climbs every migration, a non-superuser role is created,
and the refusals are observed as PostgreSQL raises them — the CHECK on a row whose `org_id`
disagrees with its path, the policy on a write into another tenant, an empty result for another
tenant's exact path and id, zero rows touched by an update or a delete aimed across the boundary,
nothing at all for a connection that has named nobody. The migration ladder's catalogue now reads
`pg_policy` and the row-security flags, so the reverse of 0009 is held to land exactly where 0008
left the database, and the reverse checker classifies the six statement kinds this migration
introduced. `schema.ts`, the latest snapshot and the migration are pinned to agree: `enableRLS`,
the one policy, the one CHECK, and FORCE — which drizzle does not record, so the migration states
it and the ladder holds it.

Not done and said so: the relational tables carry `organization_id NOT NULL` and every query
names it, and they have no policy yet. The device is the same and it is the next migration; the
document store is where the product's records live, which is why it went first.

Live at commit 108cf61: migration 0009 applied to the live database as the owner role (9 → 10); `db:verify` read `documents` row security enabled and forced, the one policy `documents_tenant`, the application role neither BYPASSRLS nor superuser, every table readable and writable by it, ALL CHECKS PASSED; the booted server answered health `ok` with the schema MATCHED at 10 of 10 and served `GET /api/leads`, `GET /api/campaigns`, `GET /api/deliverability` (the tenantless `oauth_connections` read), a campaign tick and the circuit-breaker state through the store that now names the tenant per statement, with a clean boot log; a direct connection as the application role saw 0 rows under `organizations/` unnamed, and 0 when named `org_nobody` and asking for `org_1`'s paths — the live `documents` table holds no rows today, so the visibility comparison there is 0 against 0 and the visibility proof is the suite's on a real engine; the reverse was then run (dry run, then `--confirm`: 10 → 9) and the migration re-applied (9 → 10), `db:verify` passing again.

Mutation: 11/11 — the store opening a transaction without naming the tenant, a tenantless path naming a default organisation, transactional reads not naming theirs, the migration without FORCE, the reverse forgetting the policy, a policy showing every row, a policy letting a named connection write anywhere, the CHECK absent, the reverse checker blind to policies, `schema.ts` without `enableRLS`, `schema.ts` without the policy.

### S25 — a quote exists, and it is the only price the reply may state

Everything downstream of a quote was built and nothing upstream of it: a `Quote` type with its
state machine, a binding rule, a pricing context that withholds list prices when a customer has
an approved offer, an auditor that clears stated amounts against it — and no quote was ever
written. The reader the composer used read a relational table nothing inserted into, keyed by a
contact id from a different population than the API's contacts, and the auditor was told
NOT_LOOKED_UP on every run, which it treated correctly as "cannot clear".

The capability is a service over the document store. A quote is found by the customer's
normalised email, because that is the one identity both populations of contact share, and it
sits under the organisation's path, so the tenant policy from S4 covers it without a line of
new code. A line names a tier and a component, and the unit price is read from the price book:
there is no field on the contract for an amount, so the single-price-source rule is obeyed by
construction rather than by review, and `pricingVersion` records which book the quote was made
from. The moves ask the machine, which is the shared transition map named as an entity. The
approval rule is the binding rule read backwards — an APPROVED quote that names nobody binds
nothing, so approving without a name is refused, and in production the operator gate refuses an
unattributed approval before the service is reached. Approving supersedes the previous offer in
the same transaction, because two offers in force is a state the composer could not be trusted
with. An expired draft cannot be approved; a quote that would already have expired cannot be
made.

The reply path is what makes this a capability rather than a table. The pipeline looks quotes
up by the sender's address before it plans, and passes down what it found in the two shapes the
downstream code was written for: the context bundle gets the quotes and no longer declares QUOTE
unavailable when the lookup ran; the composer gets the quote in force and a reader that throws
when the lookup did not run, which is its existing refuse-to-draft-pricing path; the auditor is
told LOADED with the quote or NOT_LOOKED_UP with nothing. An unreadable store is NOT_LOOKED_UP
with the reason — never an empty LOADED, because "we looked and found none" is a fact that
licenses list pricing and "we could not look" is not.

Unchanged and said so: the checkout amount stays configuration with no default (the earlier
finding on this row), and reconciling a charge with an approved quote is a step this makes
possible rather than performs. The console has no quote screen; quoting is an API call. The
relational `quote_snapshots` table is retired with a note where somebody grepping for it will
read it.

Live at commit 7fa755b: the served document read 98 routes with 15 contracts, the quote line item carrying exactly `component`, `quantity`, `tierId` and `additionalProperties: false`; `POST /api/contacts/nope/quotes` refused `{}` naming both missing fields, a line carrying `unitPrice` (unrecognised key), and an unknown tier (`expected "standard"`), each 400, and a valid body with 404 (no such contact); `GET /api/contacts/nope/quotes` 404; `GET /api/quotes/quo_nope` and its `submit`, `approve` and `withdraw` each 404 (no such quote); nothing was written and the boot log was clean.

Mutation: 9/11, then 2/2 — approval by nobody recorded under a placeholder, a second approval leaving the first in force, a quote born APPROVED, the setup fee charged whatever the component, an unreadable store reported as LOADED, an expired quote still in force, an already-expired quote made, an amount accepted on a line, the approve route skipping the gate, all killed; the two survivors were the pipeline's hand-offs (the auditor told NOT_LOOKED_UP whatever was found, the composer given no quote), which no behavioural suite calls the pipeline to observe — they are pinned by text on the four hand-offs now, the device `livePath.invariant` uses for this module, and both were killed on the re-run.
...
|---|---:|---|
| `VERIFIED` | **49** | S1, S2, S3, S4, S5, S6, S7, S8, S9, S10, S11, S12, S13, S14, S15, S16, S17, S18, S19, S20, S21, S22, S23, S24, S25, S26, S27, S28, S29, S30, S31, S32, S33, S34, S35, S36, S37, S38, S39, S40, S41, S42, S43, S44, S45, S46, S47, S48, S49 |
| `IMPLEMENTED_UNVERIFIED` | **0** | — |
| `PARTIAL` | **0** | — |
| `NOT_STARTED` | **0** | — |
| `NOT_ASSESSED` | **0** | all 49 sections are present in the assessment data |

49 + 0 + 0 + 0 = **49 rows**. *(Recomputed 2026-09-12 — see §1ad and §1ae. S10 moved to VERIFIED because the audit gate it was graded on now exists; S40 because `src/` no longer imports from `server/` at all; S6 because every status write on its eight entities now asks the map. The 2026-09-08 figures are in §1ac.)*
...
|---|---|---|---|---|---|
| S1 | Active code graph: dead modules, competing owners, untracked repo-mutation scripts | VERIFIED | CRITICAL | `docs/production/active-code-graph.md` (the deliverable, written); `server/services/pipeline.service.ts:5-11`; `server/gateway/actionGateway.ts:173`; `server.ts:344`; `server/routes/outbox.routes.ts:13` | **Closed 2026-09-12.** The note here said the graph was stale and regenerating it was the remaining work; the row's title named three things — dead modules, competing owners, unaccounted scripts — and each is an assertion now, not a paragraph. `docs/production/active-code-graph.md` is GENERATED by `server/build/codeGraph.ts` (`npm run code-graph`): every import followed from `server.ts` and `src/main.tsx` — relative, `@/` alias, dynamic, comments blanked by the route table's scanner — and from every script that package.json or `.github/workflows/*.yml` invokes; a module is live, operational-only or dead by what reaches it, and the scanner REFUSES a specifier or a script entry that resolves to nothing rather than leaving it out. `codeGraph.invariant` (23) regenerates the document and fails on any difference; holds the dead set at ZERO — the ten files the first regeneration named were deleted (eight agents nothing called, three of them having a model invent investors, leads and partners; a jurisdiction policy nothing read; a component nothing rendered), and with them eight legacy prompt sites and eight `safeGenerateJSON` sites, both ratchets lowered to what remains (2 and 1); pins the importers of each external capability — Gmail, Calendar, the HTTP client, the model client, the gateway, the outbox — and of each outward SDK (`@google/genai`, `stripe`, `firebase-admin`, `pg`) to an exact recorded set, so a new owner is a decision the suite sees; holds that no file under `scripts/` is reached by nothing (three operator tools only a comment knew how to run are named in package.json: `db:apply`, `outbox:backfill-version`, `org:claim`); and holds that every live module is tracked by git, the defect of `a391ad4`. The package table found a competing owner on its first run: `pitchBattleAgent` built its own `GoogleGenAI` and called the model past `geminiClient` — past S22's run log, S37's ledger and the prompt-authority ratchet, which could not see it — with the user's pitch interpolated into its system instruction; it calls through the client now, the transcript and the pitch as fenced blocks. Not derived and not claimed: who OUGHT to own a capability; the suite pins what is. Live at commit e0481dc: a fresh `git clone` of HEAD into an empty directory — 532 tracked files, no `.env`, lockfile in sync with package.json (`npm ci --dry-run`) — compiled with zero type errors and passed the full gate there (84 suites, 2,110 tests) with nothing from the working tree, which is the assertion the integrity finding could not make at the time; a first attempt installed dependencies onto a system drive with 431 MB free and died of ENOSPC, so the second borrowed the working tree's install through a junction and tested the clone's own sources. Mutation: 16/16 — 15 of 16 on the first run; the survivor, G3 (comments not blanked), was equivalent for every commented-out form the scanner test then used, since a line-start regex never matched `// import`; the test gained a block comment whose inner line begins with `import` and a commented-out dynamic `import()`, which the unblanked scan follows, and G3 was killed on the re-run; the sixteenth is the environmental probe, an untracked file imported by `server.ts`, which the git-tracked invariant failed on, naming the file. |
...
| S4 | Tenant integrity at database level | VERIFIED | CRITICAL | `server/tenancy/orgScope.ts`; `server/middleware/tenant.ts`; `server/db/schema.ts`; `firestore.rules:5` | **Closed 2026-09-12.** The remainder as written was two things and one of them had already moved: "`firestore.rules` remains open, so the datastore enforces nothing" was true of Firestore, and the datastore is PostgreSQL now (§1u) — no code in this repository reads or writes Firestore data, the rules file in the tree is the closed P0.0 rewrite, and deploying it to close the old project data is the owner's console step, named as such. What remained was the substance: the PostgreSQL side enforced nothing about tenancy either. The document store was tenant-safe by construction only — the path names the organisation, `org_id` is derived from the path, every query carries the path — and a query with a defective predicate, a script on the application's credentials, or a future statement on `documents` could read or write any tenant's rows. Migration 0009 makes the database hold it: a CHECK that `org_id` is the tenant the path names (the `tenantOf` rule, in SQL); row-level security on `documents`, FORCED so the owning role is subject to it, with one policy — a connection sees, and may write, only the rows of the tenant it has NAMED through `app.org_id`, plus the tenantless top-level documents, and a connection that names no tenant sees no tenant; and the store names the tenant to the database before every statement, from the path it is about to touch (`SELECT set_config('app.org_id', …, true)` inside the statement's transaction, per operation inside `runTransaction`). `tenantRowSecurity.invariant` (13) runs every migration on a real engine (PGlite), creates a non-superuser role — a superuser bypasses every policy, which is why the application role must never be one — and observes the refusals as PostgreSQL raises them: the CHECK on a mismatched `org_id`, the policy on a write into another tenant, an empty result for another tenant's exact path and id, zero rows touched by an update or delete aimed across the boundary, and nothing at all for a connection that names no tenant; it also pins that `schema.ts`, the latest snapshot and the migration agree (`enableRLS`, the one policy, the one CHECK, FORCE). The migration ladder (S5) now catalogues policies and the row-security flags, so the reverse of 0009 is held to land exactly on 0008; the reverse checker classifies the six new statement kinds. `db:verify` reads the catalogue — enabled, forced, the policy, the application role neither BYPASSRLS nor superuser. Not done and said so: the relational tables (`conversations`, `messages`, …) carry `organization_id NOT NULL` and every query names it, but they have no policy yet; the same device applies and is the next migration. Live at commit 108cf61: migration 0009 applied to the live database as the owner role (9 → 10); `db:verify` read `documents` row security enabled and forced, the one policy `documents_tenant`, the application role neither BYPASSRLS nor superuser, every table readable and writable by it, ALL CHECKS PASSED; the booted server answered health `ok` with the schema MATCHED at 10 of 10 and served `GET /api/leads`, `GET /api/campaigns`, `GET /api/deliverability` (the tenantless `oauth_connections` read), a campaign tick and the circuit-breaker state through the store that now names the tenant per statement, with a clean boot log; a direct connection as the application role saw 0 rows under `organizations/` unnamed, and 0 when named `org_nobody` and asking for `org_1`'s paths — the live `documents` table holds no rows today, so the visibility comparison there is 0 against 0 and the visibility proof is the suite's on a real engine; the reverse was then run (dry run, then `--confirm`: 10 → 9) and the migration re-applied (9 → 10), `db:verify` passing again. Mutation: 11/11 — the store opening a transaction without naming the tenant, a tenantless path naming a default organisation, transactional reads not naming theirs, the migration without FORCE, the reverse forgetting the policy, a policy showing every row, a policy letting a named connection write anywhere, the CHECK absent, the reverse checker blind to policies, `schema.ts` without `enableRLS`, `schema.ts` without the policy. |
...
| S25 | Quotes / quote snapshots vs public pricing | VERIFIED | HIGH | `db/schema.ts:284-292`; `salesDecisionEngine.ts:584`, `:663`; `independentAuditor.ts` (`quoteAvailability`) | **Closed 2026-09-12.** The row's substance was that no quote was ever written, and that what was missing was the capability. `server/services/quote.service.ts` is the capability. Quotes are documents under `organizations/<org>/quotes`, keyed by the customer's normalised email — the one identity the API's contacts and the inbound pipeline's resolved senders share — and covered by the tenant policy S4 put on the store. A line names a tier and a component (`monthly`, `setupFee`) and the unit price is read from the price book, so a quote cannot state an amount the book does not hold, and the contract has no amount field to refuse (`createQuoteSchema`, strict, the tier an enum of the book's ids); `pricingVersion` is a digest of the book at the time of quoting. Every move asks the QUOTE machine — the shared transitions, now an entity in `stateMachines.ts` like every other status write. Approval needs an identified approver, because the shared binding rule refuses an APPROVED quote that names nobody, and in production the operator gate refuses an unattributed approval; approving one quote supersedes any other APPROVED quote for the customer in the same transaction, so there is never a moment with two offers in force; an expired draft cannot be approved and a quote that would already have expired cannot be made. Routes: `POST`/`GET /api/contacts/:id/quotes` (the contact's email, never the body's), `GET /api/quotes/:id`, `POST /api/quotes/:id/submit|approve|withdraw`. The reply path now looks quotes up by the sender's address and passes LOADED or NOT_LOOKED_UP down — the context bundle carries the quotes, the composer receives the quote in force and a reader that throws when the lookup did not run (its existing refuse-pricing path), and the auditor is told what was found, so its stated-amounts check can clear a figure against an approved quote for the first time; an unreadable store is NOT_LOOKED_UP with the reason, never an empty LOADED. The relational `quote_snapshots` table is RETIRED (its contact key pointed at another population of contacts) and `ledgers.service` no longer reads it. Unchanged and said so: the checkout amount is configuration with no default, and reconciling it with an approved quote is a step this makes possible rather than performs; the console has no quote screen, so quoting is an API call. `quotes.invariant` (18). Live at commit 7fa755b: the served document read 98 routes with 15 contracts, the quote line item carrying exactly `component`, `quantity`, `tierId` and `additionalProperties: false`; `POST /api/contacts/nope/quotes` refused `{}` naming both missing fields, a line carrying `unitPrice` (unrecognised key), and an unknown tier (`expected "standard"`), each 400, and a valid body with 404 (no such contact); `GET /api/contacts/nope/quotes` 404; `GET /api/quotes/quo_nope` and its `submit`, `approve` and `withdraw` each 404 (no such quote); nothing was written and the boot log was clean. Mutation: 9/11, then 2/2 — approval by nobody recorded under a placeholder, a second approval leaving the first in force, a quote born APPROVED, the setup fee charged whatever the component, an unreadable store reported as LOADED, an expired quote still in force, an already-expired quote made, an amount accepted on a line, the approve route skipping the gate, all killed; the two survivors were the pipeline's hand-offs (the auditor told NOT_LOOKED_UP whatever was found, the composer given no quote), which no behavioural suite calls the pipeline to observe — they are pinned by text on the four hand-offs now, the device `livePath.invariant` uses for this module, and both were killed on the re-run. |
| S26 | Campaign contact safety (suppression, caps, quiet hours, reply-stops) | VERIFIED | CRITICAL | `src/App.tsx:710-716`; `actionGateway.ts:49-107`; `outbox.worker.ts:50,57-73` | **Closed 2026-09-12.** The note said the guards, the table and the recipient machine were in place and the one thing missing was the thing that runs a sequence: no enrolment record, no per-contact state, no scheduler. `server/services/campaignEngine.service.ts` is that thing. Enrolment (`POST /api/campaigns/:id/recipients`, a strict contract naming 1–500 contacts) creates one document per (campaign, contact) under `campaignEnrolments` with the id derived from the pair, so a second enrolment is a refused create that overwrites nothing — a recipient who replied stays replied. A tick (`POST /api/campaigns/run-tick`; `POST /api/autopilot/run-cycle-now`, which said "success" and ran nothing until S39 and refused after it; and the scheduler, which starts only when `CAMPAIGN_SCHEDULER_ENABLED` is exactly "true" and says why when it does not) cancels what belongs to a COMPLETED campaign; reconciles what the outbox has reported — PROCESSED advances the recipient to the next step, due its delay after the CONFIRMED send, DEAD_LETTER or CANCELLED fails it with the job named, in flight waits; stops a suppressed or unsubscribed contact and one who has written since enrolment; and dispatches what is due, oldest first, at most 25 per tick — the fourteen guards evaluated on inputs computed from what exists (the contact document, the recipient's own send history across campaigns, the other enrolments, the relational conversations, the contact's stated time zone), the step rendered with every merge tag resolved or refused naming the tag, and the job enqueued with the key `campaign:<campaign>:<contact>:<step>`, the inbound-version stamp the worker's integrity check demands, and a HUMAN_REVIEW hold unless the campaign is FULL_AUTOPILOT. Every refusal is written on the recipient with the guards named. An input that cannot be computed is NOT RUN and refuses (§14): a contact with no `timeZone` cannot receive a step — so `timeZone` is a contact field now, IANA only, stated at creation or by `POST /api/contacts/:id/time-zone` — and a contact with no consent record is refused before a job exists, because the gateway would refuse it after. The relational `campaign_recipients` table is RETIRED: its foreign keys point at relational campaigns and contacts, and both are documents. Nothing here sends — a tick enqueues, the worker dispatches through the gateway, `REAL_EMAIL_SEND_ENABLED` is false; a sequence can now RUN, which is different from a message leaving. Not done and said so: only EMAIL steps are performed (a LinkedIn or call step is recorded as not performed and the sequence moves on); the reply stop reads the relational conversations by contact id and any activity since enrolment counts; the console has no enrolment control yet, so enrolment is an API call. `campaignSequence.invariant` (13), `campaignEngine.invariant` (27), `campaignScheduler.invariant` (11). Live at commit 8b51919: the served document read 92 routes with 14 contracts, the enrolment contract carrying `minItems 1, maxItems 500, additionalProperties false`; `POST /api/campaigns/nope/recipients` refused `{}`, `{contactIds:[]}` and `{contactIds:["x"],sendNow:true}` with 400 naming the field, and `{contactIds:["x"]}` with 404 (no such campaign); `POST /api/contacts/nope/time-zone` refused `+06:00` with 400 (not an IANA zone) and `Europe/London` with 404 (no such contact); `POST /api/campaigns/run-tick` and `POST /api/autopilot/run-cycle-now` each ran a real tick over the live organisation — 0 campaigns, nothing reconciled, dispatched, refused or stopped, no errors — the second reporting the scheduler as not running with the reason (`CAMPAIGN_SCHEDULER_ENABLED` is not "true"), which the boot log also said; nothing was enrolled and no row was written. Mutation: 17/17 — consent skipped, noon assumed for a contact with no zone, every step PENDING, the key without the step, the job without its inbound version, suppression not stopped, the last step reported as a wait, the next step scheduled from the tick rather than the confirmed send, a LinkedIn step treated as email, an unfilled tag replaced by nothing, the per-tick bound and the per-domain count of this tick's sends removed, the scheduler starting whatever the flag says, a second enrolment overwriting, a fixed-offset zone accepted, a reply not stopping the sequence, the guards evaluated and ignored. |
...
| S39 | Monolith: ~75 route registrations against empty decomposition folders | VERIFIED | CRITICAL | `server.ts:309-344`, `:760-826`, `:62`, `:193-195` | **Closed 2026-09-12.** The note counted ~70 of ~75 endpoints inline, dead controller and repository layers, ~30 success stubs, zero validation, webhooks registered only in production. The layers and the webhook registration had been dealt with in earlier passes; what remained was seventy-two API routes inline in a 2,094-line `startServer`, eight fixed-answer stubs the P1.13 guardrail could not see, and four body-reading routes the route table could not see. Every API route now lives in a router under `server/routes/` (fifteen new files, twenty-four in all), mounted from a `server.ts` of 224 lines that is a composition root — middleware, mounts, static serving, listen — and nothing else. The move was one mechanical, text-preserving pass, and the twenty-three suites that pinned route text in `server.ts` were re-pointed to the router that holds it, most through the route table's own handler reader (`handlerTextOf`), so a pin now also says WHERE the code lives. `decomposition.invariant` (21) holds the shape: the only inline registration is the SPA catch-all; every router file is mounted exactly once and every mount has a file; the guardrails that name `server.ts` read the `server/` tree; and — found by the live probe, not by the gate — the first import of `server.ts` is the `.env` loader, because `environment.ts` reads `DATABASE_URL` as it evaluates and the move had pruned that import along with the readiness route that used its exports, leaving a server whose store was null and whose every tenant-scoped request answered `TENANT_REVOCATION_UNVERIFIABLE`. The eight fixed answers — `{ intentConfidence: 0.9 }`, `{ decision: "Proceed" }`, a placeholder sender identity — and the autopilot-settings echo refuse with `NOT_IMPLEMENTED`, and `check-no-fabricated-success` scans the routers and forbids a handler whose every value is a literal, in either form. The route table follows a `const fn = async (` handler now — `setCampaignStatus` and `setOpportunityStage` are arrows, so their four routes had been read as a registration line and reported as reading no body — and those four routes carry strict contracts (the state named, an optional version, nothing else), validated before the read, which a behavioural test proves by the status a bad body earns on a record that does not exist: 400, not 404. Coverage in the artefact: 88 routes, 12 with a contract, 14 without (was 16), 2 raw. Not done and said so: the fourteen no-schema routes remain named in the suite. Live at commit 0f286cf: the first boot after the move answered `TENANT_REVOCATION_UNVERIFIABLE` on every tenant-scoped route — the pruned `.env` loader, above; with the import restored, the booted server served `GET /api/health` from `server/routes/health.routes.ts:55` (200, the `x-source` in the served document), `GET /api/leads` and `GET /api/campaigns` 200, `POST /api/campaigns/nope/status` with `{status:"BOGUS"}` 400 and with `{status:"ACTIVE",extra:1}` 400 naming the key, `/toggle` with a legal state on a missing campaign 404, `PUT /api/pipeline/nope/stage` 400 then 404 likewise, the nine fixed answers 501 `NOT_IMPLEMENTED`, `POST /api/webhooks/gmail` without a token 401 `WEBHOOK_VERIFICATION_FAILED`, `POST /api/settings` with an unknown field still 400; the served coverage read 88 routes, 12 with a contract, 14 without, 2 raw. Mutation: 12/13, then 2/2 — R5 (the fixed-answer patterns dropped from the guardrail) survived a text pin on the constant's name; the suite now runs the script against a planted file of its own, in both forms, and R5 was killed on the re-run beside a new mutant that moved the `.env` loader below the rate limiter import; the run also planted a fixed answer in a router file in each form and the script failed naming the file. |
...
| **P0.14** | *(DONE — `express.raw` is mounted before the global JSON parser and signatures are verified. `webhookVerification.invariant`. The same ordering trap is why S35's CSP parser is mounted early in §1ab.)* **Fix the webhook body-parser ordering and verify signatures.** Mount `express.raw` before the global `express.json()`; implement DocuSign HMAC and Pub/Sub JWT verification; gate the CONFIRMED write on current meeting status; move both webhook routes out of the production-only branch. | Ordering matters and the previous draft got it right: fixing the parser **without** adding HMAC converts a dead endpoint into a working forgery endpoint. Never land the parser fix alone. | S33, S39, S6 |
| **P0.15** | *(PARTLY DONE, and my first note on this row was wrong — §1b DID land work here, which I missed by grepping only the sections after it. What landed: `/api/meetings/:id/process-payment` no longer returns a fabricated `{success:true}`, and Stripe checkout is gated on `REAL_PAYMENT_ENABLED` and the durable circuit breaker. What did NOT: `PAYMENT_CREATE` still falls through the gateway execution switch to `Unsupported action type`, so payments bypass the ownership lock, the action log and reconciliation. See §1ac for the amount.)* **Route payments through the gateway.** Implement `PAYMENT_CREATE`, move the Stripe checkout behind it, correct the `unit_amount`, persist `checkout.session.completed`, and delete the `process-payment` success stub. | Financial correctness. Depends on P0.8 so a payment cannot be simulated, and on P0.3 so it can be halted. | S25, S1, S6, S46 |
